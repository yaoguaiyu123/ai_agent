...
# 一个 Streamlit 应用，用于通过简单的聊天界面与 LangGraph Agent 进行交互
# 该应用主要包含三个函数，并且它们都以 async 方式运行：

# - main() - 初始化 Streamlit 应用并搭建整体结构
# - draw_messages() - 绘制一组聊天消息，可以是重新展示已有消息，
#   也可以是流式显示新消息
# - handle_feedback() - 绘制反馈组件，并记录用户提交的反馈

# 该应用大量使用 AgentClient 与 Agent 的 FastAPI endpoints 进行交互
...
# note 获取user_id，用于登录之前的“匿名账号”
def get_or_create_user_id() -> str:
    """从 session state 或 URL 参数中获取 user ID；如果不存在，则创建一个新的"""

    # st.session_state 是一个字典对象，存储于网页会话里的临时内存
    # 检查 session state 中是否已经存在 user_id
    ...
    # 如果没有找到，则生成一个新的 user_id
    user_id = str(uuid.uuid4())

    # 将其保存到当前 session 的 session state 中
    st.session_state[USER_ID_COOKIE] = user_id

    # 同时添加到 URL 参数中，方便添加书签或分享链接
    st.query_params[USER_ID_COOKIE] = user_id

    return user_id

# tip @st.cache_data(ttl=600, show_spinner=False) 是 Streamlit 的缓存装饰器
# 第一次调用fetch_user_threads_cached，会去后端获取，第二次（600s以内），则在本地缓存中找
@st.cache_data(ttl=600, show_spinner=False)
def fetch_user_threads_cached(
    base_url: str, user_id: str, agent_id: str | None = None, limit: int = 20
) -> UserThreads:
    """
    从后端获取某个用户的会话线程列表（threads）
    """
...
async def main() -> None:
    ...
    if len(messages) == 0:
        match agent_client.agent:
            case "chatbot":
                WELCOME = "你好！我是一个简单的聊天机器人，你可以问我任何问题！"
            case "research-assistant":
                WELCOME = "你好！我是一个 AI 研究助手，支持网页搜索功能。你可以问我任何问题！"
            case _:
                WELCOME = "你好！我是一个 AI Agent，你可以问我任何问题！"
