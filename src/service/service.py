...
async def stream(user_input: StreamInput, agent_id: str = DEFAULT_AGENT) -> StreamingResponse:
    """
    将 agent 对用户输入的响应以流式方式返回，包括中间消息和 token。

    如果未提供 agent_id，将使用默认 agent。使用 thread_id 来持久化并延续多轮对话。
    run_id 关键字参数也会附加到所有消息上，用于记录反馈。使用 user_id 来跨多个线程持久化并延续对话。

    将 stream_tokens 设为 false 可以返回中间消息，但不会逐 token 输出。
    """
...
async def threads(
    ...
    """
    按最近更新时间排序，列出某个用户的对话线程列表。


    user_id 由调用方声明，不会对照请求中的凭据进行校验，因此任何持有 bearer token 
    的人都可以列出任意用户的线程——这与 /history 接口的信任模型一致。在终端用户能够访
    问此接口之前，请自行在其前面添加鉴权逻辑。
    """
    agent: AgentGraph = get_agent(agent_id)
    checkpointer = getattr(agent, "checkpointer", None)
    if not checkpointer:
